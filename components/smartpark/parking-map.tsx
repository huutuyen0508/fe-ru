...
export function ParkingMap({ mode = 'readonly', compact = false, variant = 'default', floorId: initialFloorId = 'B1', selectableSlotIds = [], selectedSlotId, onSelectSlot }: ParkingMapProps) {
  ...
  function selectSlot(slot: ParkingSlot) { if (mode === 'select') { if ((slot.status === 'available' || slot.isMine === true) && selectableSlotIds.includes(slot.id)) { setSelected(slot); onSelectSlot?.(slot) }; return }; setSelected(slot); if (mode === 'admin') setEditing(true) }

  return <div className={compact ? 'min-w-0' : 'mx-auto max-w-[1440px]'}>
    <div className="flex justify-center border-b"><div className="flex gap-2" role="tablist" aria-label="Các tầng đỗ xe">{sharedFloors.map((item) => <button key={item.id} role="tab" aria-selected={floorId === item.id} onClick={() => { setFloorId(item.id); setSelected(null); setEditing(false) }} className={`rounded-md border px-4 py-2 text-sm font-medium transition-colors ${floorId === item.id ? 'border-primary text-primary' : 'border-border text-foreground hover:border-primary/50'}`}>{item.label}</button>)}</div></div>
